...
public class DESTR {
    ...
    // 枚举类型，对应Pascal中的TDesMode
    public enum DesMode {
        ENCRYPT, DECRYPT
    }

    // 初始值置IP
    private static final byte[] BitIP = {
    ...
    // 逆初始置IP-1
    private static final byte[] BitCP = {
    ...
    // 位选择函数E
    private static final int[] BitExp = {
    ...
    // 置换函数P
    private static final byte[] BitPM = {
    ...
    // 选择置换PC-2
    private static final byte[] BitPMC2 = {
            13, 16, 10, 23, 0, 4,
            2, 27, 14, 5, 20, 9,
            22, 18, 11, 3, 25, 7,
            15, 6, 26, 19, 12, 1,
    ...
    // DES数据处理
    private static void desData(DesMode desMode, byte[] inData, byte[] outData) {
        ...
        if (desMode == DesMode.ENCRYPT) {
            for (int i = 0; i < 16; i++) {
                System.arraycopy(outData, 0, temp, 0, 4); // temp = Ln
                System.arraycopy(outData, 4, outData, 0, 4); // Ln+1 = Rn
    ...
    public static String encryptStr(String str, String key) {
        if (str.length() > 0 && str.charAt(str.length() - 1) == 0) {
            throw new RuntimeException("Error: the last char is NULL char.");
        }
        ...
        for (int i = 0; i < str.length() / 8; i++) {
            ...
            desData(DesMode.ENCRYPT, strByte, outByte);
            for (int j = 0; j < 8; j++) {
                result.append((char) (outByte[j] & 0xFF));
            }
        }
    ...
    public static String decryptStr(String str, String key) {
        ...
        for (int i = 0; i < str.length() / 8; i++) {
            ...
            desData(DesMode.DECRYPT, strByte, outByte);
            for (int j = 0; j < 8; j++) {
                result.append((char) (outByte[j] & 0xFF));
            }
        }
    ...
    public static String encryptStrHex(String str, String key) {
        String tempResult = encryptStr(str, key);
        StringBuilder result = new StringBuilder();

        for (int i = 0; i < tempResult.length(); i++) {
            String temp = Integer.toHexString(tempResult.charAt(i) & 0xFF);
            if (temp.length() == 1) {
                temp = "0" + temp;
            }
            result.append(temp);
